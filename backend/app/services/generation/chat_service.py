...
class ChatService:
    ...
    async def chat(
        ...
        try:
            ...
            err_str = str(e)
            logger.error(f"Generation failed: {err_str}", exc_info=True)
            # Show friendly rate-limit message instead of generic error
            if "429" in err_str or "quota" in err_str.lower() or "ResourceExhausted" in err_str:
                answer = (
                    "⚠️ I'm currently rate-limited by the Gemini API (free tier). "
                    "The API allows only 5 requests per minute. Please wait 30–60 seconds and try again. "
                    "Your document was processed successfully and is ready to be queried!"
                )
            else:
